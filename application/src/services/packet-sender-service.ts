...
function isPrivateIpAddr(ip: string): boolean {
  return (
    ipInCidr(ip, '10.0.0.0/8') || ipInCidr(ip, '172.16.0.0/12') || ipInCidr(ip, '192.168.0.0/16')
  );
}
...
function pickRoute(
  routes: RoutingTableEntry[] | undefined,
  dstIp: string,
...
function matchPort(port: number | undefined, pattern: string): boolean {
  ...
  if (pattern.includes(',')) return pattern.split(',').some((p) => parseInt(p.trim(), 10) === port);
  if (pattern.includes('-')) {
...
export function tracePacket(topology: NetworkTopology, req: SendPacketRequest): TraceResult {
  const id = uuidv4();
  ...
  if (srcNode.config.powered === false) {
    return makeError(
      ...
      timestamp,
      req,
      `${srcNode.config.hostname ?? srcNode.label} is powered off`,
    );
  }
  if (dstNode.config.powered === false) {
    return makeError(
      ...
      timestamp,
      req,
      `${dstNode.config.hostname ?? dstNode.label} is powered off`,
    );
  }
  ...
  if (!srcIp) {
    if (isInternet(srcNode)) srcIp = '198.51.100.1';
      ...
      return makeError(
        ...
        timestamp,
        req,
        `${srcNode.config.hostname ?? srcNode.label} has no IP address — power it on / run DHCP first`,
      );
  }
  if (!dstIp) {
    if (isInternet(dstNode)) dstIp = '203.0.113.10';
      ...
      return makeError(
        ...
        timestamp,
        req,
        `${dstNode.config.hostname ?? dstNode.label} has no IP address`,
      );
  }
  ...
  const pathNodes = nodePath
    .map((pid) => topology.nodes.find((n) => n.id === pid))
  ...
  const crossSubnet =
    !!srcNet && !!dstNet && !(srcNet.net === dstNet.net && srcNet.prefix === dstNet.prefix);
  const sVlan = accessVlan(srcNode),
    dVlan = accessVlan(dstNode);
  ...
  const sZone = srcNode.config.zone,
    dZone = dstNode.config.zone;
  ...
  for (let i = 1; i < nodePath.length; i++) {
      ...
      if (nextNodeF && isInternet(nextNodeF) && isPrivateIpAddr(srcIp) && !isPrivateIpAddr(dstIp)) {
        const pub =
          node.config.interfaces
            ?.map((x) => x.ipAddress)
            .find((ip) => ip && !isPrivateIpAddr(ip)) ?? '203.0.113.1';
        fwNat = `  ·  NAT ${srcIp} -> ${pub}`;
        srcIp = pub;
      }
      hops.push({
    ...
    // ── Router / Layer-3 switch ───────────────────────────────────────────────
      ...
      if (
        !isDestination &&
        nextNodeR &&
        isInternet(nextNodeR) &&
        isPrivateIpAddr(srcIp) &&
        !isPrivateIpAddr(dstIp)
      ) {
        const publicIp =
          node.config.interfaces
            ?.map((x) => x.ipAddress)
    ...
    // ── Layer-7 delivery devices (Load Balancer / Reverse Proxy / API Gateway) ─
      ...
      if (isDestination) {
        ...
        if (
          (req.protocol === 'tcp' || req.protocol === 'udp') &&
          req.dstPort &&
          services.length > 0
        ) {
        ...
        const nextHop = nodePath[i + 1]
          ? (topology.nodes.find((n) => n.id === nodePath[i + 1])?.config.hostname ??
            nodePath[i + 1])
          : 'upstream';
        hops.push({
          step: i,
          ...
          nodeType: node.type,
          action: 'route',
          detail: `${role} ${nodeName}: forwarding (L7) -> ${nextHop}`,
          latencyMs: hopLatency,
          edgeId,
        });
      }
  ...
  if (success && zoneCross) {
    ...
    if (last)
      last.detail += `  —  warning: crosses the ${sZone} -> ${dZone} zone boundary with no firewall in the path`;
  }
...
function makeError(
  ...
  return {
    ...
    hops: [
      {
        step: 0,
        nodeId: req.srcNodeId,
        nodeName: req.srcNodeId,
        nodeType: 'unknown',
        action: 'no_route',
        detail: message,
        latencyMs: 0,
