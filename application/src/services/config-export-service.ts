...
export function deviceRunningConfig(topo: NetworkTopology, node: NetworkNode): string {
  ...
  for (const i of c.interfaces ?? []) {
    ...
    if (isSwitch) {
      ...
      const p = prefixOf(i);
      L.push(
        ` ip address ${i.ipAddress} ${p !== null ? prefixToMask(p) : (i.subnetMask ?? '255.255.255.0')}`,
      );
    }
  ...
  if (c.firewallRules?.length) {
    ...
    for (const r of [...c.firewallRules].sort((a, b) => a.priority - b.priority)) {
      ...
      L.push(
        ` ${act} ${proto} ${aclAddr(r.srcIp)} ${aclAddr(r.dstIp)}${dport}  ! #${r.priority} ${r.direction}${r.hits != null ? ` (${r.hits} hits)` : ''}${state}`,
      );
    }
    L.push('!');
  }
  ...
  if (c.services?.length) {
    for (const s of c.services)
      L.push(`! service ${s.name} ${s.protocol}/${s.port} ${s.enabled ? 'enabled' : 'disabled'}`);
    L.push('!');
  }
