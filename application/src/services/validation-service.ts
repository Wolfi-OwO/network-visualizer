...
const ENDPOINTS_SKIP_LINKCHECK = new Set([
  'switch',
  'hub',
  'l3switch',
  'wifiap',
...
export function validateTopology(topo: NetworkTopology): ValidationReport {
  ...
  const add = (
    ...
    category: string,
    message: string,
    ref?: { nodeId?: string; nodeLabel?: string; edgeId?: string },
  ) => {
  ...
  for (const [ip, owners] of ipOwners) {
    if (owners.length > 1) {
      ...
      add(
        'error',
        'duplicate-ip',
        `Duplicate IP ${ip} assigned to ${owners.length} interfaces (${who})`,
        { nodeId: owners[0].node.id, nodeLabel: label(owners[0].node) },
      );
    }
  }
  ...
  for (const [mac, owners] of macOwners) {
    if (owners.length > 1) {
      add('error', 'duplicate-mac', `Duplicate MAC ${mac} on ${owners.map(label).join(', ')}`, {
        nodeId: owners[0].id,
        nodeLabel: label(owners[0]),
      });
    }
  }
    ...
    for (const d of defaults) {
      ...
      if (ips.length > 0 && !reachable) {
        add(
          'error',
          'gateway-unreachable',
          `Default gateway ${d.gateway} is not in any of ${label(n)}'s connected subnets`,
          { nodeId: n.id, nodeLabel: label(n) },
        );
      }
    }
  }
  ...
  for (const n of topo.nodes) {
    ...
    const hasDefault = (n.config.routingTable ?? []).some(
      (r) => r.destination === '0.0.0.0' || r.type === 'default',
    );
    if (hasIp && !hasDefault) {
      add(
        'info',
        'no-default-route',
        `${label(n)} has no default route — it can only reach its own subnet`,
        { nodeId: n.id, nodeLabel: label(n) },
      );
    }
  }
  ...
  for (const n of topo.nodes) {
    if ((degree.get(n.id) ?? 0) === 0) {
      add('warning', 'isolated-node', `${label(n)} is not connected to any link`, {
        nodeId: n.id,
        nodeLabel: label(n),
      });
    }
  ...
  for (const e of topo.edges) {
    ...
    if (!share) {
      add(
        ...
        `${label(a)} <-> ${label(b)} are directly connected but share no common subnet`,
        { edgeId: e.id, nodeId: a.id },
      );
    }
  }
  ...
  for (const n of topo.nodes) {
    const rules = (n.config.firewallRules ?? [])
      .filter((r) => r.enabled)
      .slice()
      .sort((x, y) => x.priority - y.priority);
    const isAny = (v: string) => v === 'any' || v === '*' || v === '0.0.0.0/0';
    const matchAll = (r: (typeof rules)[number]) =>
      r.protocol === 'any' &&
      isAny(r.srcIp) &&
      isAny(r.srcPort) &&
      isAny(r.dstIp) &&
      ...
      for (let j = i + 1; j < rules.length; j++) {
        if (dir === 'both' || rules[j].direction === dir || rules[j].direction === 'both') {
          add(
            'warning',
            'shadowed-rule',
            `${label(n)} firewall rule #${rules[j].priority} ("${rules[j].description}") is shadowed by match-all rule #${rules[i].priority} and never matches`,
            { nodeId: n.id, nodeLabel: label(n) },
          );
        }
      }
