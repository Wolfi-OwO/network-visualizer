import type { NetworkTopology, NetworkNode, NetworkEdge, NetworkInterface } from '../types/index.js'
...
export interface ArpEntry { ip: string; mac: string; iface: string; type: 'dynamic' | 'static' }
export interface MacEntry { vlan: number; mac: string; port: string; type: 'dynamic' | 'static' }
export interface LeaseEntry { ip: string; mac: string; hostname: string; state: string; lease: string }
export interface OspfNeighbor { neighborId: string; state: string; address: string; iface: string }
export interface StpPort { port: string; neighbor: string; role: 'root' | 'designated' | 'blocking'; state: 'forwarding' | 'blocking' }
export interface StpInfo { bridgeId: string; rootBridgeId: string; isRoot: boolean; ports: StpPort[] }
export interface AclEntry { seq: number; action: string; protocol: string; src: string; dst: string; direction: string; hits: number; enabled: boolean }
export interface NatEntry { protocol: string; insideLocal: string; insideGlobal: string; outsideGlobal: string }
...
function ifaces(n: NetworkNode): NetworkInterface[] { return n.config.interfaces ?? [] }
function ipv4Ifaces(n: NetworkNode): NetworkInterface[] { return ifaces(n).filter((i) => ipToInt(i.ipAddress) !== null) }
function firstMac(n: NetworkNode): string | undefined { return ifaces(n).find((i) => i.macAddress)?.macAddress }
function nameOf(n: NetworkNode): string { return n.config.hostname || n.label || n.id }
function randPort(): number { return 1024 + Math.floor(Math.random() * 64000) }

interface Neighbor { node: NetworkNode; edge: NetworkEdge; port: string }
function neighborsOf(topo: NetworkTopology, node: NetworkNode, byId: Map<string, NetworkNode>): Neighbor[] {
  const out: Neighbor[] = []
  ...
  for (const local of mine) {
    ...
    for (const other of topo.nodes) {
      ...
      for (const oi of ipv4Ifaces(other)) {
        ...
        arp.push({ ip: oi.ipAddress!, mac: oi.macAddress ?? 'incomplete', iface: local.name, type: 'dynamic' })
      }
    }
  }
  return arp
...
function buildMacTable(topo: NetworkTopology, node: NetworkNode, byId: Map<string, NetworkNode>): MacEntry[] {
...
function gatewayIps(topo: NetworkTopology): Set<string> {
  ...
  for (const n of topo.nodes) {
    for (const r of n.config.routingTable ?? []) {
      if ((r.destination === '0.0.0.0' || r.type === 'default') && r.gateway && r.gateway !== '0.0.0.0') gws.add(r.gateway)
    }
  }
  return gws
}
...
function buildLeases(topo: NetworkTopology, node: NetworkNode): LeaseEntry[] {
  const myNets = ipv4Ifaces(node).map((i) => ({ ip: i.ipAddress!, prefix: prefixOf(i) })).filter((x) => x.prefix !== null)
  ...
  for (const h of topo.nodes) {
    ...
    for (const hi of ipv4Ifaces(h)) {
      ...
      if (!inMine) continue
      leases.push({ ip: hi.ipAddress!, mac: hi.macAddress ?? '—', hostname: nameOf(h), state: 'Active', lease: '23:59:00' })
      break
    }
  }
  return leases
...
function buildOspf(topo: NetworkTopology, node: NetworkNode, byId: Map<string, NetworkNode>): OspfNeighbor[] {
  ...
  for (const nb of neighborsOf(topo, node, byId)) {
    ...
    for (const local of mine) {
      const prefix = prefixOf(local)
      if (prefix === null) continue
      const peer = ipv4Ifaces(nb.node).find((i) => inSubnet(i.ipAddress!, local.ipAddress!, prefix) && i.ipAddress !== local.ipAddress)
      if (!peer) continue
      const peerHigher = (ipToInt(peer.ipAddress) ?? 0) > (ipToInt(local.ipAddress) ?? 0)
      out.push({ neighborId: peer.ipAddress!, state: peerHigher ? 'FULL/DR' : 'FULL/BDR', address: peer.ipAddress!, iface: local.name })
      break
    }
  }
  return out
...
function buildStp(topo: NetworkTopology, node: NetworkNode, byId: Map<string, NetworkNode>): StpInfo {
  ...
  for (const e of topo.edges) {
    if (adj.has(e.source) && adj.has(e.target)) { adj.get(e.source)!.add(e.target); adj.get(e.target)!.add(e.source) }
  }
  ...
  while (stack.length) {
    const cur = stack.pop()!
    for (const nx of adj.get(cur) ?? []) if (!comp.has(nx)) { comp.add(nx); stack.push(nx) }
  }
  const rootNode = switches
  ...
  const ports: StpPort[] = neighborsOf(topo, node, byId).map((nb) => {
    if (!L2_TYPES.has(nb.node.type)) return { port: nb.port, neighbor: nameOf(nb.node), role: 'designated' as const, state: 'forwarding' as const }
    if (isRoot) return { port: nb.port, neighbor: nameOf(nb.node), role: 'designated' as const, state: 'forwarding' as const }
    const nbDist = dist.get(nb.node.id) ?? Infinity
    if (nbDist === myDist - 1) return { port: nb.port, neighbor: nameOf(nb.node), role: 'root' as const, state: 'forwarding' as const }
    if (nbDist >= myDist) return { port: nb.port, neighbor: nameOf(nb.node), role: 'designated' as const, state: 'forwarding' as const }
    return { port: nb.port, neighbor: nameOf(nb.node), role: 'blocking' as const, state: 'blocking' as const }
  })
...
function buildAcl(node: NetworkNode): AclEntry[] {
  return (node.config.firewallRules ?? [])
    ...
    .map((r) => ({
      seq: r.priority, action: r.action, protocol: r.protocol,
      src: `${r.srcIp}:${r.srcPort}`, dst: `${r.dstIp}:${r.dstPort}`,
      direction: r.direction, hits: r.hits ?? 0, enabled: r.enabled,
    }))
}
...
function buildNat(topo: NetworkTopology, node: NetworkNode): NatEntry[] {
  ...
  const insideHosts = topo.nodes
    ...
    .flatMap((n) => ipv4Ifaces(n).filter((i) => isPrivate(i.ipAddress!)).map((i) => i.ipAddress!))
  const outsides = topo.nodes.flatMap((n) => ipv4Ifaces(n).filter((i) => !isPrivate(i.ipAddress!)).map((i) => i.ipAddress!))
    .filter((ip) => ip !== wan.ipAddress)
  const pickOutside = () => outsides.length ? outsides[Math.floor(Math.random() * outsides.length)] : '93.184.216.34'
...
// ── Entry point ──────────────────────────────────────────────────────────────
export function controlPlaneForNode(topo: NetworkTopology, nodeId: string): ControlPlaneReport | null {
  ...
  if (L2_TYPES.has(node.type)) { report.macTable = buildMacTable(topo, node, byId); report.stp = buildStp(topo, node, byId) }
  if (ROUTER_TYPES.has(node.type) || node.type === 'firewall') report.ospfNeighbors = buildOspf(topo, node, byId)
