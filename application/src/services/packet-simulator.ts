...
function rtt(a: Host, b: Host): number {
  const offNet =
    a.kind === 'internet' || b.kind === 'internet' || a.kind === 'dns' || b.kind === 'dns';
...
function tcpSegment(
  ...
  const header = [
    ...
    ack & 0xff,
    0x50,
    flagsByte, // data offset 5 (no options)
    (window >> 8) & 0xff,
    window & 0xff,
    0x00,
    0x00, // checksum placeholder
...
function emitTcp(o: TcpOpts): void {
  ...
  schedule(o.at, {
    ...
    protocol: o.protocol,
    info: o.info,
    color: getProtocolColor(o.protocol),
    ethernet,
    ip: baseIpLayer(o.src.ip, o.dst.ip, 6, 'TCP', seg.bytes.length, ttl, ipId, ipH.checksum),
    tcp,
    http: o.http,
    tls: o.tls,
...
function emitUdp(o: UdpOpts): void {
  ...
  const udp: UdpLayer = {
    srcPort: o.srcPort,
    dstPort: o.dstPort,
    length: 8 + o.payload.length,
    checksum: hex16(dg.checksum),
  };
...
function emitDnsLookup(
  ...
  emitUdp({
    ...
    srcPort: sport,
    dstPort: 53,
    payload: randomPayload(28),
    protocol: 'DNS',
    info: `Standard query ${hex16(txid)} A ${domain}`,
    dns: {
      transactionId: hex16(txid),
      flags: '0x0100',
      isResponse: false,
  ...
  emitUdp({
    ...
    srcPort: 53,
    dstPort: sport,
    payload: randomPayload(60),
    protocol: 'DNS',
    info: `Standard query response ${hex16(txid)} A ${domain} A ${resolvedIp}`,
    dns: {
      transactionId: hex16(txid),
      flags: '0x8180',
      isResponse: true,
...
function emitWebSession(startAt: number, client: Host, server: Host, secure: boolean): void {
  ...
  emitTcp({
    ...
    window: 64240,
    protocol: 'TCP',
    info: `${sport} -> ${dport} [SYN] Seq=${cSeq} Win=64240 Len=0 MSS=1460`,
  });
  t += r / 2;
  emitTcp({
    at: t,
    ...
    window: 65160,
    protocol: 'TCP',
    info: `${dport} -> ${sport} [SYN, ACK] Seq=${sSeq} Ack=${(cSeq + 1) >>> 0} Win=65160 Len=0 MSS=1460`,
  });
  cSeq = (cSeq + 1) >>> 0;
  sSeq = (sSeq + 1) >>> 0;
  t += r / 2;
  emitTcp({
    ...
    flags: { ack: true },
    protocol: 'TCP',
    info: `${sport} -> ${dport} [ACK] Seq=${cSeq} Ack=${sSeq} Win=64240 Len=0`,
  });
  ...
  for (let i = 0; i < rounds; i++) {
    ...
    emitTcp({
      ...
      flags: { psh: true, ack: true },
      payload: reqPayload,
      protocol: secure ? 'TLS' : 'HTTP',
      info: secure ? 'Application Data' : `${method} ${uri} HTTP/1.1`,
      http: secure
        ? undefined
        : {
            isRequest: true,
            ...
            uri,
            version: 'HTTP/1.1',
            headers: { Host: server.name, 'User-Agent': 'Mozilla/5.0', Accept: '*/*' },
          },
    });
    ...
    emitTcp({
      ...
      flags: { ack: true },
      protocol: 'TCP',
      info: `${dport} -> ${sport} [ACK] Seq=${sSeq} Ack=${cSeq} Win=65160 Len=0`,
    });
    ...
    emitTcp({
      ...
      payload: respPayload,
      protocol: secure ? 'TLS' : 'HTTP',
      info: secure ? 'Application Data' : `HTTP/1.1 ${status} ${statusMsg[status]}`,
      http: secure
        ? undefined
        : {
            isRequest: false,
            version: 'HTTP/1.1',
            statusCode: status,
            statusMessage: statusMsg[status],
            headers: {
              'Content-Type': 'text/html',
  ...
  emitTcp({
    ...
    protocol: 'TCP',
    info: `${sport} -> ${dport} [FIN, ACK] Seq=${cSeq} Ack=${sSeq} Win=64240 Len=0`,
  });
  cSeq = (cSeq + 1) >>> 0;
  t += r / 2;
  emitTcp({
    ...
    flags: { fin: true, ack: true },
    protocol: 'TCP',
    info: `${dport} -> ${sport} [FIN, ACK] Seq=${sSeq} Ack=${cSeq} Win=65160 Len=0`,
  });
  sSeq = (sSeq + 1) >>> 0;
  t += r / 2;
  emitTcp({
...
function emitIcmp(
  ...
  schedule(at, {
    ...
    icmp: {
      type: reply ? 0 : 8,
      typeName: reply ? 'Echo (ping) reply' : 'Echo (ping) request',
      code: 0,
      checksum: hex16(cs),
      identifier,
      sequenceNumber: seq,
...
function emitArp(at: number, src: Host, dst: Host, reply: boolean): void {
  ...
  const arp = [
    ...
    ...macToBytes(src.mac),
    ...ipToOctets(src.ip),
    ...macToBytes(reply ? dst.mac : '00:00:00:00:00:00'),
    ...ipToOctets(dst.ip),
  ];
  ...
  schedule(at, {
    ...
    arp: {
      ...
      protocolSize: 4,
      opcode: reply ? 2 : 1,
      opcodeName: reply ? 'reply' : 'request',
      senderMac: src.mac,
      senderIp: src.ip,
      targetMac: reply ? dst.mac : '00:00:00:00:00:00',
      targetIp: dst.ip,
    },
    hexDump: generateHexDump(rawBytes),
    rawBytes,
...
function emitDhcpDora(at: number, client: Host): void {
  ...
  const phases: Array<{ phase: 'Discover' | 'Offer' | 'Request' | 'ACK'; fromClient: boolean }> = [
    { phase: 'Discover', fromClient: true },
    { phase: 'Offer', fromClient: false },
    { phase: 'Request', fromClient: true },
    { phase: 'ACK', fromClient: false },
  ];
  let t = at;
  for (const { phase, fromClient } of phases) {
    ...
    const fields = [
      ...
      {
        key: 'Your (client) IP',
        value: phase === 'Offer' || phase === 'ACK' ? offered : '0.0.0.0',
      },
      { key: 'DHCP Server', value: GATEWAY.ip },
      { key: 'Subnet Mask', value: '255.255.255.0' },
      { key: 'Router', value: GATEWAY.ip },
      { key: 'DNS', value: '8.8.8.8, 1.1.1.1' },
      { key: 'Lease Time', value: '86400s (1 day)' },
    ];
    emitUdp({
      at: t,
      src: { ...src, ip: srcIp },
      dst: { ...dst, ip: dstIp },
      srcPort: fromClient ? 68 : 67,
      dstPort: fromClient ? 67 : 68,
      payload: randomPayload(240),
      protocol: 'DHCP',
    ...
    protoViews: [
      {
        ...
        fields: [
          { key: 'Version', value: '4' },
          { key: 'Mode', value: 'client (3)' },
          { key: 'Stratum', value: 'unspecified (0)' },
        ],
      },
    ],
  });
  emitUdp({
    at: at + rtt(client, GATEWAY),
    src: GATEWAY,
    dst: client,
    srcPort: 123,
    dstPort: sport,
    ...
    protoViews: [
      {
        ...
        fields: [
          { key: 'Version', value: '4' },
          { key: 'Mode', value: 'server (4)' },
          { key: 'Stratum', value: 'secondary reference (2)' },
...
function emitSnmp(at: number): void {
  ...
  emitUdp({
    ...
    payload: randomPayload(60),
    protocol: 'SNMP',
    info: 'get-request 1.3.6.1.2.1.1.3.0',
    protoViews: [
      {
        ...
        fields: [
          ...
          { key: 'OID', value: '1.3.6.1.2.1.1.3.0 (sysUpTime)' },
        ],
      },
    ],
  });
...
function emitOspfHello(at: number): void {
  ...
  const rawBytes = [
    ...ethernetBytes(src.mac, '01:00:5e:00:00:05', 0x0800),
    ...ipH.bytes,
    ...payload,
  ];
  schedule(at, {
    ...
    protoViews: [
      {
        ...
        fields: [
          ...
          { key: 'Dead Interval', value: '40 seconds' },
        ],
      },
    ],
    hexDump: generateHexDump(rawBytes),
...
function emitStpBpdu(at: number): void {
  ...
  schedule(at, {
    ...
    capturedLength: rawBytes.length,
    protocol: 'STP',
    info: 'Conf. Root = 32768/00:1a:2b:3c:4d:00  Cost = 0',
    color: getProtocolColor('STP'),
    ethernet: {
      srcMac: src.mac,
      dstMac: STP_MAC,
      etherType: '0x0026',
      etherTypeName: 'IEEE 802.3',
    },
    protoViews: [
      {
        name: 'Spanning Tree Protocol',
        summary: '(Rapid/Configuration BPDU)',
        fields: [
          { key: 'Protocol', value: 'STP (0x0000)' },
          { key: 'Version', value: 'RSTP (2)' },
          { key: 'Root', value: '32768 / 00:1a:2b:3c:4d:00' },
...
function emitLldp(at: number): void {
  ...
  schedule(at, {
    ...
    capturedLength: rawBytes.length,
    protocol: 'LLDP',
    info: `${src.name} Port ${port} TTL=120`,
    color: getProtocolColor('LLDP'),
    ethernet: { srcMac: src.mac, dstMac: LLDP_MAC, etherType: '0x88cc', etherTypeName: 'LLDP' },
    protoViews: [
      {
        name: 'Link Layer Discovery Protocol',
        fields: [
          { key: 'Chassis ID', value: `MAC (${src.mac})` },
          { key: 'Port ID', value: port },
          { key: 'TTL', value: '120 seconds' },
          { key: 'System Name', value: src.name },
...
function emitMdns(at: number, src: Host): void {
  ...
  emitUdp({
    at,
    src,
    dst: { ...src, ip: '224.0.0.251' },
    srcPort: 5353,
    dstPort: 5353,
    payload: randomPayload(40),
    protocol: 'mDNS',
    info: `Standard query 0x0000 PTR ${name}`,
    multicastMac: '01:00:5e:00:00:fb',
    protoViews: [
      {
        name: 'Multicast DNS',
        fields: [
...
function emitSsdp(at: number, src: Host): void {
  emitUdp({
    at,
    src,
    dst: { ...src, ip: '239.255.255.250' },
    srcPort: randInt(49152, 65535),
    dstPort: 1900,
    payload: randomPayload(120),
    protocol: 'SSDP',
    info: 'M-SEARCH * HTTP/1.1',
    multicastMac: '01:00:5e:7f:ff:fa',
    protoViews: [
      {
        name: 'Simple Service Discovery Protocol',
        fields: [
          { key: 'Method', value: 'M-SEARCH' },
          { key: 'Host', value: '239.255.255.250:1900' },
          { key: 'ST', value: 'ssdp:all' },
          { key: 'Man', value: '"ssdp:discover"' },
        ],
      },
    ],
  });
...
function startUserSession(at: number): void {
  ...
  if (roll < 0.55) {
    ...
    if (Math.random() < 0.5) emitMdns(at, client);
    else emitSsdp(at, client);
  }
}
...
function due(key: string, now: number, intervalMs: number, jitter = 0): boolean {
  if (nextDue[key] === undefined) {
    nextDue[key] = now + Math.random() * intervalMs;
    return false;
  }
  if (now >= nextDue[key]) {
    nextDue[key] = now + intervalMs + (jitter ? randFloat(-jitter, jitter) : 0);
    return true;
  }
  return false;
}
