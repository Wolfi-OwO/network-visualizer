...
function assertValidTopologyPatch(body: unknown): void {
  ...
  if ('nodes' in body && !validNodes(body.nodes))
    throw new BadRequestError('Invalid nodes: each needs id, type, position {x,y}, config');
  if ('edges' in body && !validEdges(body.edges))
    throw new BadRequestError('Invalid edges: each needs id, source, target');
}
function assertValidNodeBody(body: unknown): void {
  ...
  if (body.config !== undefined && !isObj(body.config))
...
async function resolve(id: string, owner: string): Promise<NetworkTopology | null> {
  return id === 'default'
    ? networkService.getOrCreateDefault(owner)
    : networkService.getTopology(id, owner);
}

// ── Topology collection ───────────────────────────────────────────────────────
export async function listTopologies(req: Request, res: Response): Promise<void> {
  const owner = ownerOf(req);
  if ((await networkService.getAllTopologies(owner)).length === 0)
    await networkService.getOrCreateDefault(owner);
  const items = (await networkService.getAllTopologies(owner)).map((t) =>
    withLinks(t, topologyLinks(t.id)),
  );
...
export async function createTopology(req: Request, res: Response): Promise<void> {
  ...
  res
    .status(201)
    .location(`/api/networks/${topology.id}`)
    .json(withLinks(topology, topologyLinks(topology.id)));
}
...
export async function deleteTopology(req: Request, res: Response): Promise<void> {
  if (!(await networkService.deleteTopology(req.params.id, ownerOf(req))))
    throw new NotFoundError('Topology not found');
  res.status(204).end();
}
...
export async function listVersions(req: Request, res: Response): Promise<void> {
  ...
  res.json({
    _links: { self: { href: `/api/networks/${topology.id}/versions` } },
    count: items.length,
    items,
  });
}
...
export async function deleteNode(req: Request, res: Response): Promise<void> {
  if (!(await networkService.deleteNode(req.params.id, req.params.nodeId, ownerOf(req))))
...
export async function deleteEdge(req: Request, res: Response): Promise<void> {
  if (!(await networkService.deleteEdge(req.params.id, req.params.edgeId, ownerOf(req))))
