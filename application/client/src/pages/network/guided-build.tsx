...
function clientsOnline(nodes: NetworkNode[]) {
  return nodes.filter((n) => ENDPOINT_TYPES.includes(n.type) && n.config.interfaces?.[0]?.ipAddress)
    .length
}
...
export default function GuidedBuild({
    ...
    return [
      ...
      },
      {
        ...
        hint: 'Drag three endpoints (PCs, a laptop, a printer — your choice) and wire each one to a switch. They arrive powered off and without an address, just like unboxed hardware.',
          ...
          ENDPOINT_TYPES.reduce((a, t) => a + count(nodes, t), 0) >= 3 &&
      ...
      <div style={panelStyle}>
        <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[var(--border)]">
          ...
          <span className="text-[12px] font-semibold text-[var(--text-primary)] flex-1">
            Hands-on: Build TechCorp's network
          </span>
          <button
            onClick={onClose}
        ...
        <div className="px-3 py-3 text-[12px] leading-relaxed text-[var(--text-secondary)] space-y-2">
          <p>
            Build <b>TechCorp HQ</b> exactly the way real enterprise sites are layered, from the
            Internet edge inward:
          </p>
          ...
          <ul className="list-disc list-inside space-y-0.5 text-[var(--text-secondary)]">
            <li>
              An <b>edge router</b> on the ISP uplink handling the routing
            </li>
            <li>
              A <b>perimeter firewall</b> behind it inspecting all LAN traffic
            </li>
            <li>
              <b>DHCP + DNS</b> in the server segment on the core switch
            </li>
            <li>
              An <b>access switch</b> fanning out to PCs, laptop, printer
            </li>
            <li>
              A <b>DMZ</b>: the public web server on its own firewall leg
            </li>
            <li>
              Finally: rules, records, and <b>powering it all on</b> (DHCP DORA)
            </li>
          </ul>
          <p className="text-[var(--text-muted)]">
            Each step is checked as you complete it. Best to start from a blank canvas.
          </p>
        </div>
        <div className="flex gap-2 px-3 py-2.5 border-t border-[var(--border)]">
          ...
          <button onClick={() => setPhase('tasks')} className="btn-ghost text-[11px]">
            Use current canvas
          </button>
        </div>
      </div>
  ...
  return (
    <div style={panelStyle}>
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[var(--border)]">
        {allDone ? (
          <PartyPopper size={14} className="text-[var(--green)]" />
        ) : (
          <Hammer size={14} className="text-[var(--accent)]" />
        )}
        <span className="text-[12px] font-semibold text-[var(--text-primary)] flex-1">
            ...
            return (
              <div
                ...
                className={[
                  ...
                  isCurrent
                    ? 'bg-[rgba(88,166,255,0.08)] border border-[var(--accent)]/40'
              ...
              >
                <div className="flex items-center gap-2">
                  {t.done ? (
                    ...
                    <Circle
                      size={14}
                      className={`shrink-0 ${isCurrent ? 'text-[var(--accent)]' : 'text-[var(--bg-600)]'}`}
                    />
                  )}
                  <span
                    className={[
                ...
                {isCurrent && (
                  <p className="text-[10px] text-[var(--text-muted)] leading-relaxed mt-1 ml-6">
                    {t.hint}
                  </p>
                )}
              </div>
          ...
          {allDone && (
            <div className="mt-2 p-2.5 rounded-md bg-[rgba(63,185,80,0.1)] border border-[var(--green)]/40">
              ...
              <p className="text-[10px] text-[var(--text-secondary)] mt-1 leading-relaxed">
                You've built a site the way it's really wired: desks on an <b>access switch</b>,
                servers on the <b>core</b>, a <b>perimeter firewall</b> guarding the LAN behind the{' '}
                <b>edge router</b>, and a <b>DMZ</b> web server on its own firewall leg. Use{' '}
                <b>Send Packet</b> to trace a PC out to the Internet (watch it cross the firewall,
                then the edge router), or send TCP 443 from the Cloud to the DMZ server to see your
                allow rule fire. Turn on
                <b> Live</b> to watch DNS lookups and web traffic flow across the layers.
              </p>
              <button
                onClick={onClose}
                className="btn-success w-full justify-center text-[11px] mt-2"
              >
                Finish
              </button>
            </div>
