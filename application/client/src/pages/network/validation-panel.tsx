...
export default function ValidationPanel({
  ...
  return (
    <div className="absolute bottom-3 left-3 w-[min(20rem,calc(100%-1.5rem))] max-h-[60%] flex flex-col rounded-lg popover shadow-xl z-20 overflow-hidden">
      ...
      {report && (
        <div className="flex items-center gap-3 px-3 py-1.5 text-[11px] border-b border-[var(--border)]">
          <span
            className={`inline-flex items-center gap-1 ${report.ok ? 'text-[var(--green)]' : 'text-red-400'}`}
          >
            {report.ok ? (
              <>
              ...
              <>
                <XCircle size={12} /> {report.counts.error} error
                {report.counts.error === 1 ? '' : 's'}
              </>
            )}
          </span>
          <span className="text-[var(--text-muted)]">
            {report.counts.warning} warn · {report.counts.info} info
          </span>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {report && report.findings.length === 0 && !error && (
          <div className="p-4 text-xs text-[var(--text-muted)] flex items-center gap-2">
            <ShieldCheck size={14} className="text-[var(--green)]" /> All checks passed — no issues
            found.
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                <span className="block text-[11px] text-[var(--text-primary)] leading-snug">
                  {f.message}
                </span>
