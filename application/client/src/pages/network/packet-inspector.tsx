...
function LayerRow({
  ...
  return (
    <div className="border-b border-[var(--border)]/50">
      ...
      >
        {open ? (
          <ChevronDown size={11} className="mt-0.5 shrink-0 text-[var(--text-muted)]" />
        ) : (
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-900)] border-l border-[var(--border)] w-full">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--border)] bg-[var(--bg-950)] shrink-0">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-[var(--text-muted)] font-mono">
            {packets.length} packet(s) captured
          </div>
        </div>
        <button
      ...
      <div className="shrink-0 max-h-[34%] overflow-y-auto border-b border-[var(--border)]">
        <table className="w-full text-[10px] font-mono">
          ...
          <tbody>
            {packets.length === 0 && (
              <tr>
                <td colSpan={5} className="px-2 py-3 text-center text-[var(--text-muted)]">
                  Click a moving packet on the canvas to capture it.
                </td>
              </tr>
            )}
            {packets.map((p, i) => {
              ...
              return (
                <tr
                  ...
                  className={[
                    'cursor-pointer',
                    isSel ? 'bg-[rgba(88,166,255,0.15)]' : 'hover:bg-[var(--bg-800)]',
                  ].join(' ')}
                >
                  <td className="px-2 py-1 text-[var(--text-muted)]">{i + 1}</td>
                  <td
                    className="px-1 py-1 text-[var(--text-secondary)] truncate max-w-[90px]"
                    title={`${p.srcName} ${p.srcIp}`}
                  >
                    {p.srcIp || p.srcName}
                  </td>
                  <td
                    className="px-1 py-1 text-[var(--text-secondary)] truncate max-w-[90px]"
                    title={`${p.dstName} ${p.dstIp}`}
                  >
                    {p.dstIp || p.dstName}
                  </td>
                  <td className="px-1 py-1 font-semibold" style={{ color: protoColor(p.protocol) }}>
                    {p.protocol}
                  </td>
      ...
      {sel ? (
        <>
          <div
            className="px-2 py-1 text-[10px] font-mono text-[var(--text-secondary)] bg-[var(--bg-950)] border-b border-[var(--border)] truncate"
            title={sel.info}
          >
          ...
          <div className="shrink-0 max-h-[28%] overflow-y-auto border-t border-[var(--border)] bg-[var(--bg-950)] p-2">
            <pre className="text-[9.5px] font-mono leading-[1.5] text-[var(--text-muted)] whitespace-pre">
              {sel.payloadHex}
            </pre>
          </div>
        </>
        ...
        <div className="flex-1 flex items-center justify-center text-[11px] text-[var(--text-muted)] p-4 text-center">
          Turn on <b className="mx-1 text-[var(--text-secondary)]">Live</b> traffic, then click a
          moving packet to decode it.
        </div>
      )}
    </div>
  )
