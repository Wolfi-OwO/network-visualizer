...
type PropsTab =
  'info' | 'interfaces' | 'routing' | 'firewall' | 'vlans' | 'dhcp' | 'dns' | 'services'
...
function InterfaceRow({
  ...
  return (
    <div className="card p-0 overflow-hidden">
      <div
        className="flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-[var(--bg-700)]"
        onClick={() => setOpen((o) => !o)}
      >
        {open ? (
        ...
        <span className="text-[11px] font-mono font-semibold text-[var(--text-primary)] flex-1">
          {iface.name || 'eth?'}
        </span>
        <button
          onClick={(e) => {
            e.stopPropagation()
            onUpdate({ ...iface, status: iface.status === 'up' ? 'down' : 'up' })
          }}
        ...
        <button
          ...
          className="text-[var(--red)] opacity-60 hover:opacity-100 p-0.5"
        >
          <Trash2 size={10} />
        </button>
      </div>
      {open && (
        <div className="px-2 pb-2 space-y-1.5 border-t border-[var(--border)]">
          <div className="pt-1.5 grid grid-cols-2 gap-1.5">
            <Field label="Name">
              <TinyInput value={iface.name} onChange={(v) => upd('name', v)} placeholder="eth0" />
            </Field>
            <Field label="IP">
              <TinyInput
                value={iface.ipAddress ?? ''}
                onChange={(v) => upd('ipAddress', v)}
            ...
            <Field label="VLAN">
              <TinyInput
                ...
                onChange={(v) =>
                  onUpdate({ ...iface, vlan: v === '' ? undefined : parseInt(v, 10) || undefined })
                }
                type="number"
                placeholder="1 (untagged)"
              />
            </Field>
          </div>
          <Field label="Desc">
...
function RouteRow({
  ...
  const typeColors: Record<string, string> = {
    ...
    static: 'text-[var(--accent)]',
    default: 'text-[var(--yellow)]',
    dynamic: 'text-[var(--purple)]',
  }
  return (
    <tr className="border-b border-[var(--border)]/30 hover:bg-[var(--bg-700)] group">
      <td className="px-1 py-1">
        <input
          className="input text-[10px] h-5 py-0 px-1 font-mono w-full"
      ...
      <td className="px-1 py-1">
        <input
          ...
          onChange={(e) => upd('interface', e.target.value)}
          placeholder="eth0"
        />
      </td>
      <td className="px-1 py-1">
        <input
          className="input text-[10px] h-5 py-0 px-1 font-mono w-12"
          type="number"
          value={route.metric}
      ...
      <td className="px-1 py-1">
        <select
          className="select text-[10px] h-5 py-0 px-1"
          value={route.type}
      ...
      <td className="px-1 py-1">
        <button
          onClick={onDelete}
          className={`opacity-0 group-hover:opacity-100 ${typeColors[route.type] ?? ''} transition-opacity`}
        >
...
function FirewallRuleRow({
  ...
  return (
    <div
      className={`card p-0 overflow-hidden border-l-2 ${rule.action === 'allow' ? 'border-l-[var(--green)]' : 'border-l-[var(--red)]'} ${rule.enabled ? '' : 'opacity-50'}`}
    >
      <div
        className="flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-[var(--bg-700)]"
        onClick={() => setOpen((o) => !o)}
      >
        ...
        <span className={`text-[10px] font-bold ${actionColor} w-10`}>
          {rule.action.toUpperCase()}
        </span>
        <span className="badge text-[9px] bg-[var(--bg-700)] text-[var(--text-secondary)]">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <Field label="Priority">
              <TinyInput
                ...
                onChange={(v) => upd('priority', parseInt(v) || 1)}
                type="number"
              />
            </Field>
            <Field label="Action">
              <select
                className="select text-[10px] h-6 py-0 px-2 w-full"
                value={rule.action}
            ...
            <Field label="Src IP">
              <TinyInput value={rule.srcIp} onChange={(v) => upd('srcIp', v)} placeholder="any" />
            </Field>
            <Field label="Src Port">
              <TinyInput
                value={rule.srcPort}
                onChange={(v) => upd('srcPort', v)}
                placeholder="any"
              />
            </Field>
            <Field label="Dst IP">
              <TinyInput value={rule.dstIp} onChange={(v) => upd('dstIp', v)} placeholder="any" />
            </Field>
            <Field label="Dst Port">
              <TinyInput
                value={rule.dstPort}
                onChange={(v) => upd('dstPort', v)}
                placeholder="any"
              />
            </Field>
  ...
  return (
    <div className={`card p-0 overflow-hidden ${svc.enabled ? '' : 'opacity-55'}`}>
      ...
      >
        ...
        />
        <span className="text-[11px] font-semibold text-[var(--text-primary)] flex-1 truncate">
          {svc.name || 'service'}
        </span>
        <span className="badge text-[9px] bg-[var(--bg-700)] text-[var(--text-secondary)] font-mono">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <Field label="Port">
              <TinyInput
                ...
                onChange={(v) => upd('port', parseInt(v) || 0)}
                type="number"
              />
            </Field>
            <Field label="Protocol">
  ...
  return (
    <tr className="border-b border-[var(--border)]/30 hover:bg-[var(--bg-700)] group">
      ...
      <td className="px-1 py-1">
        ...
        >
          {(['A', 'AAAA', 'CNAME', 'MX', 'NS', 'TXT'] as const).map((t) => (
            <option key={t} value={t}>
              {t}
            </option>
          ))}
      ...
      <td className="px-1 py-1">
        <input
          className="input text-[10px] h-5 py-0 px-1 font-mono w-14"
          type="number"
          value={record.ttl}
          ...
          <button
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent('netviz:togglePower', { detail: { id: node.id } }),
              )
            }
            ...
            style={{
              background: powered ? 'rgba(63,185,80,0.15)' : 'var(--bg-800)',
              color: powered ? 'var(--green)' : 'var(--text-muted)',
            }}
          >
            <Power size={11} />
            {powered ? 'On' : 'Off'}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {tab === 'info' && (
          <div className="p-3 space-y-2">
            ...
            <Field label="Model">
              <TinyInput
                value={config.model ?? ''}
                onChange={(v) => push({ model: v })}
                placeholder="Cisco ISR 4321"
              />
            </Field>
            <Field label="OS">
              <TinyInput
                value={config.osType ?? ''}
                onChange={(v) => push({ osType: v })}
                placeholder="IOS 15.7"
              />
            </Field>
            <Field label="Mgmt IP">
              <TinyInput
                value={config.mgmtIp ?? ''}
                onChange={(v) => push({ mgmtIp: v })}
            ...
            <Field label="Serial">
              <TinyInput
                ...
                onChange={(v) => push({ serialNumber: v })}
            ...
            <div className="pt-2 mt-1 border-t border-[var(--border)]">
              ...
              <div className="flex flex-wrap gap-1">
                {hardwareFor(node.type).map((h) => (
                  <span
                    key={h}
                    className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-800)] border border-[var(--border)] text-[var(--text-secondary)]"
                  >
                    {h}
                  </span>
                ))}
                ...
                <thead>
                  <tr className="border-b border-[var(--border)]">
                    {['Dest', 'Mask', 'Gateway', 'Iface', 'M', 'Type', ''].map((h) => (
                      <th
                        key={h}
                        className="px-1 py-1.5 text-left text-[var(--text-muted)] font-semibold uppercase tracking-wider whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
        ...
        {tab === 'services' && (
          <div className="p-3 space-y-3">
            ...
            <div>
              <div className="text-[9px] font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-1.5">
                Quick add
              </div>
              <div className="flex flex-wrap gap-1">
            ...
            <div className="space-y-1.5">
              ...
              {services.map((s, i) => (
                <ServiceRow
                  ...
                  svc={s}
                  onUpdate={(updated) =>
                    push({ services: services.map((x, j) => (j === i ? updated : x)) })
                  }
                  onDelete={() => push({ services: services.filter((_, j) => j !== i) })}
                />
              ))}
              <button
                onClick={() => push({ services: [...services, newService()] })}
                className="btn-ghost w-full justify-center text-[10px]"
              >
            ...
            {httpOn && (
              <div className="space-y-1.5 border-t border-[var(--border)] pt-2">
                ...
                <textarea
                  ...
                  value={webPage?.body ?? ''}
                  onChange={(e) =>
                    push({ webPage: { title: webPage?.title ?? '', body: e.target.value } })
                  }
                  placeholder={'<h1>Welcome</h1>\n<p>This page is served by this device.</p>'}
                />
                <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
                  When another device sends an HTTP request to this host, the trace shows the page
            ...
            {!dhcp ? (
              <div className="text-center py-6 space-y-3">
                <p className="text-[11px] text-[var(--text-muted)]">
                  No DHCP scope configured on this device.
                </p>
                <button
                  onClick={() => push({ dhcp: defaultDhcp() })}
                  className="btn-primary text-[10px] mx-auto"
                >
              ...
              <>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    >
                      <span
                        className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${dhcp.enabled ? 'left-4' : 'left-0.5'}`}
                      />
                    </button>
                  </label>
                </div>
                <Field label="Pool Start">
                  <TinyInput
                    ...
                    onChange={(v) => push({ dhcp: { ...dhcp, poolStart: v } })}
                ...
                <Field label="Pool End">
                  <TinyInput
                    value={dhcp.poolEnd}
                    onChange={(v) => push({ dhcp: { ...dhcp, poolEnd: v } })}
                    placeholder="192.168.1.200"
                  />
                </Field>
                <Field label="Subnet Mask">
                  <TinyInput
                    value={dhcp.subnetMask}
                    onChange={(v) => push({ dhcp: { ...dhcp, subnetMask: v } })}
                    placeholder="255.255.255.0"
                  />
                </Field>
                <Field label="Gateway">
                  <TinyInput
                    value={dhcp.gateway}
                    onChange={(v) => push({ dhcp: { ...dhcp, gateway: v } })}
                    placeholder="192.168.1.1"
                  />
                </Field>
                <Field label="DNS Servers">
                  <TinyInput
                    ...
                    onChange={(v) => push({ dhcp: { ...dhcp, dnsServers: v } })}
                ...
                <Field label="Lease (h)">
                  <TinyInput
                    ...
                    onChange={(v) => push({ dhcp: { ...dhcp, leaseHours: parseInt(v) || 0 } })}
                    type="number"
                  />
                </Field>
                <button
                  onClick={() => push({ dhcp: undefined })}
                  className="btn-ghost w-full justify-center text-[10px] text-[var(--red)] mt-2"
                >
        ...
        {tab === 'dns' && (
          <div className="p-2 space-y-2">
            {!dns ? (
              <div className="text-center py-6 space-y-3">
                <p className="text-[11px] text-[var(--text-muted)]">
                  No DNS zone configured on this device.
                </p>
                <button
                  onClick={() => push({ dns: defaultDns() })}
                  className="btn-primary text-[10px] mx-auto"
                >
              ...
              <>
                <div className="flex items-center justify-between px-1">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    >
                      <span
                        className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${dns.enabled ? 'left-4' : 'left-0.5'}`}
                      />
                    </button>
                  </label>
                </div>
                <div className="px-1">
                  <Field label="Forwarders">
                    <TinyInput
                      value={dns.forwarders}
                      onChange={(v) => push({ dns: { ...dns, forwarders: v } })}
                      placeholder="8.8.8.8, 1.1.1.1"
                    />
                  </Field>
                </div>
                        ...
                        <DnsRecordRow
                          ...
                          onUpdate={(updated) =>
                            push({
                              dns: {
                                ...dns,
                                records: dns.records.map((x, j) => (j === i ? updated : x)),
                              },
                            })
                          }
                          onDelete={() =>
                            push({
                              dns: { ...dns, records: dns.records.filter((_, j) => j !== i) },
                            })
                          }
                        />
                      ))}
                ...
                <button
                  onClick={() =>
                    push({ dns: { ...dns, records: [...dns.records, newDnsRecord()] } })
                  }
                  className="btn-ghost w-full justify-center text-[10px]"
                >
                ...
                <button
                  onClick={() => push({ dns: undefined })}
                  className="btn-ghost w-full justify-center text-[10px] text-[var(--red)]"
                >
                ...
                {vlan.ports.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {vlan.ports.map((p) => (
                      <span
                        key={p}
                        className="text-[9px] font-mono text-[var(--text-muted)] bg-[var(--bg-700)] px-1 py-0.5 rounded"
                      >
                        {p}
                      </span>
                    ))}
