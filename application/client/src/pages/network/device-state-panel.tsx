...
function Section({
  ...
  return (
    <div className="mb-3">
      ...
      <div className="font-mono text-[11px] text-[var(--text-secondary)] overflow-x-auto">
        {children}
      </div>
    </div>
  )
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {!nodeId && (
          <div className="text-xs text-[var(--text-muted)]">
            Select a device on the canvas to see its live state.
          </div>
        )}
        {error && <div className="text-xs text-red-400">{error}</div>}
        {empty && (
          <div className="text-xs text-[var(--text-muted)]">
            No control-plane state for this device type.
          </div>
        )}
            ...
            <table>
              <tbody>
                ...
                {report.macTable.map((m, i) => (
                  <tr key={i}>
                    <td className={td}>{m.vlan}</td>
                    <td className={td}>{m.mac}</td>
                    <td className={td}>{m.port}</td>
        ...
        {report?.stp && (
          <Section title="Spanning tree" cmd="show spanning-tree">
            <div className="mb-1">
              bridge {report.stp.bridgeId}{' '}
              {report.stp.isRoot ? (
                <span className="text-[var(--green)]">(root)</span>
              ) : (
                <>root {report.stp.rootBridgeId}</>
              )}
            </div>
            <table>
              <tbody>
                ...
                {report.stp.ports.map((p, i) => (
                  <tr key={i}>
                    <td className={td}>{p.port}</td>
                    <td className={td}>{p.neighbor}</td>
                    <td className={td}>{p.role}</td>
                    <td
                      className={
                        p.state === 'blocking' ? 'pr-3 text-red-400' : 'pr-3 text-[var(--green)]'
                      }
                    >
                      {p.state}
                    </td>
              ...
              <tbody>
                ...
                {report.arp.map((a, i) => (
                  <tr key={i}>
                    ...
                    <td className={td}>{a.mac}</td>
                    <td className={td}>{a.iface}</td>
        ...
        {report?.ospfNeighbors && report.ospfNeighbors.length > 0 && (
          <Section title="OSPF neighbors" cmd="show ip ospf neighbor">
            <table>
              <tbody>
                <tr>
                  <th className={th}>Neighbor ID</th>
                  <th className={th}>State</th>
                  <th className={th}>Address</th>
                  <th className={th}>Interface</th>
                </tr>
                {report.ospfNeighbors.map((o, i) => (
                  <tr key={i}>
                    <td className={td}>{o.neighborId}</td>
                    <td className={td}>{o.state}</td>
                    <td className={td}>{o.address}</td>
                    <td className={td}>{o.iface}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                ...
                {report.dhcpLeases.map((l, i) => (
                  <tr key={i}>
                    ...
                    <td className={td}>{l.mac}</td>
                    <td className={td}>{l.hostname}</td>
                    <td className={td}>{l.state}</td>
                    <td className={td}>{l.lease}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                ...
                {report.acl.map((r, i) => (
                  <tr key={i} className={r.enabled ? '' : 'opacity-50'}>
                    ...
                    <td
                      className={
                        r.action === 'allow' ? 'pr-3 text-[var(--green)]' : 'pr-3 text-red-400'
                      }
                    >
                      {r.action}
                    </td>
            ...
            <table>
              <tbody>
                <tr>
                  ...
                  <th className={th}>Inside local</th>
                  <th className={th}>Inside global</th>
                  <th className={th}>Outside global</th>
                </tr>
                {report.nat.map((n, i) => (
                  <tr key={i}>
                    <td className={td}>{n.protocol}</td>
                    <td className={td}>{n.insideLocal}</td>
                    <td className={td}>{n.insideGlobal}</td>
                    <td className={td}>{n.outsideGlobal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {config && (
          <div className="mb-1">
            ...
            <pre className="font-mono text-[10px] leading-snug text-[var(--text-secondary)] bg-[var(--bg-950)] rounded p-2 overflow-x-auto whitespace-pre">
              {config}
            </pre>
          </div>
        )}
