...
export default function CIDRCalculatorPage() {
  ...
  const calculate = useCallback(
    async (value?: string) => {
      ...
      dispatch({
        type: 'patch',
        values: { loading: true, error: '', result: null, subnets: [], showSubnets: false },
      })
      try {
  ...
  const calcSubnets = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await cidrApi.subnets(
        result.networkAddress + '/' + result.cidrPrefix,
        count,
        prefix,
      )
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 backdrop-blur-xl bg-[var(--glass-bg)] border-b border-[var(--glass-border)] shrink-0">
        ...
        <span className="hidden sm:inline text-xs text-[var(--text-muted)]">
          IPv4 Subnet Calculator & Network Analyzer
        </span>
      </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {PRESETS.map((p) => (
              <button
                key={p}
                onClick={() => {
                  dispatch({ type: 'set', key: 'input', value: p })
                  calculate(p)
                }}
          ...
          <>
            ...
            <div className="animate-rise grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="card p-4 space-y-0.5">
                ...
                <ResultRow
                  label="CIDR Notation"
                  value={`${result.networkAddress}/${result.cidrPrefix}`}
                  mono
                  copyable
                />
              </div>
                ...
                <div className="pt-2 space-y-1.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-[var(--text-muted)] mt-1 flex gap-3">
                      ...
                      <span className="inline-flex items-center gap-1">
                        <span className="inline-block w-2 h-2 rounded-sm bg-[var(--text-muted)]" />{' '}
                        Host bits
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-[var(--text-muted)]">
                    Network portion (/{result.cidrPrefix})
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded bg-[var(--bg-600)]" />
                  <span className="text-[10px] text-[var(--text-muted)]">
                    Host portion ({32 - result.cidrPrefix} bits ={' '}
              ...
              <div className="mt-3 flex items-center gap-2 flex-wrap">
                ...
                <input
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: 'patch',
                      values: { subnetCount: e.target.value, subnetPrefix: '' },
                    })
                  }
                  placeholder="count"
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: 'patch',
                      values: { subnetPrefix: e.target.value, subnetCount: '' },
                    })
                  }
                  placeholder="/26"
                />
                  ...
                  <table className="w-full text-[11px]">
                    <thead>
                      <tr className="border-b border-[var(--border)]">
                        {[
                          ...
                          <th
                            key={h}
                            className="px-2 py-1.5 text-left text-[var(--text-muted)] font-semibold uppercase text-[9px] tracking-wider whitespace-nowrap"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {subnets.map((s, i) => (
                        <tr
                          key={i}
                          className="border-b border-[var(--border)]/30 hover:bg-[var(--bg-700)] transition-colors"
                        >
                          <td className="px-2 py-1 font-mono text-[var(--text-muted)]">{i + 1}</td>
                          <td className="px-2 py-1 font-mono text-[var(--accent)]">
                            {s.networkAddress}/{s.cidrPrefix}
                          </td>
        ...
        <div className="card p-4">
          ...
          <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: 'set',
                      key: 'supernetInputs',
                      value: (prev) => prev.map((s, j) => (j === i ? e.target.value : s)),
                    })
                  }
            ...
            <div className="flex gap-2">
              <button
                onClick={() =>
                  dispatch({ type: 'set', key: 'supernetInputs', value: (inp) => [...inp, ''] })
                }
