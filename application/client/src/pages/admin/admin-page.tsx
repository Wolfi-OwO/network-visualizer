    ...
    <div className="card card-hover flex items-center gap-3 p-4">
      <div
        ...
        style={{
          ...
          boxShadow: 'inset 0 0 0 1px rgba(88,166,255,0.28), 0 0 18px -6px #58a6ff',
        }}
      >
...
function UsersPanel({ currentUserId }: { currentUserId?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Could not load users — administrators only.')
    }
  }, [])
  useEffect(() => {
    load()
  ...
  const changeRole = async (u: AdminUser, role: Role) => {
    ...
    setError('')
    try {
      const { data } = await usersApi.setRole(u.id, role)
      setList((prev) => prev.map((x) => (x.id === u.id ? data : x)))
    } catch (e) {
      setError(apiError(e, 'Failed to change role'))
    } finally {
      setBusy(null)
    }
  }
  const remove = async (u: AdminUser) => {
    ...
    try {
      ...
      setError(apiError(e, 'Failed to remove user'))
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <div>
              ...
              <tr key={u.id} className="border-t border-[var(--border)]">
                ...
                <td className="px-3 py-2 text-right">
                  <button
                    onClick={() => remove(u)}
                    disabled={busy === u.id}
                    className="text-[var(--text-muted)] hover:text-[var(--red)] disabled:opacity-40"
                    title="Remove user"
                  >
                    <Trash2 size={13} />
                  </button>
      ...
      <p className="text-[10px] text-[var(--text-muted)] mt-2 leading-relaxed">
        <b>admin</b> — full access (users, roles, metrics, audit). <b>editor</b> — create &amp; edit
        own networks. <b>viewer</b> — read-only. The last administrator can't be demoted or removed.
      </p>
    </div>
  )
}
...
export default function AdminPage() {
  ...
  const download = () => {
    ...
    a.href = url
    a.download = `netviz-metrics-${Date.now()}.json`
    a.click()
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Stat
              icon={Activity}
              label="Status"
              value={m.status === 'ok' ? 'Healthy' : m.status}
              sub={`up ${fmtUptime(m.uptimeSeconds)}`}
            />
            <Stat
              icon={Cpu}
              label="Memory (heap)"
              value={`${m.process.memoryMB.heapUsed} MB`}
              sub={`rss ${m.process.memoryMB.rss} MB · ${m.process.node}`}
            />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <Stat
                ...
                value={m.auth.providers.join(', ') || 'local only'}
              />
            </div>
          </div>
        </div>
