  ...
  return (
    <div className="relative flex items-center justify-center h-full w-full p-4 sm:p-6 overflow-hidden">
      <div className="animate-rise relative w-full max-w-sm rounded-xl glass p-6 shadow-[var(--shadow-lift)]">
        ...
        <div className="space-y-2">
          {info?.providers.includes('google') && (
            <a
              ...
              className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg text-xs font-medium bg-white text-gray-800 transition-all duration-200 hover:brightness-95 hover:shadow-lg active:scale-[0.98]"
            >
          ...
          {info?.providers.includes('microsoft') && (
            <a
              href={auth.oauthUrl('microsoft')}
              className="flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg text-xs font-medium bg-[#2f2f2f] text-white border border-white/10 transition-all duration-200 hover:bg-[#3a3a3a] hover:shadow-lg active:scale-[0.98]"
            >
          ...
          {info && info.providers.length === 0 && (
            <p className="text-[10px] text-[var(--text-muted)] text-center">
              No OAuth providers configured. Set GOOGLE_CLIENT_ID / MICROSOFT_CLIENT_ID to enable
              them.
            </p>
          )}
        </div>
        ...
        <button
          onClick={() => navigate('/')}
          className="mt-4 w-full text-center text-[11px] text-[var(--text-muted)] hover:text-[var(--text-primary)]"
        >
