...
describe('auth: GET', () => {
  it('GET /api/auth/providers lists options', async () => {
  ...
  it('OAuth start endpoints 400 when the provider is not configured', async () => {
    assert.equal((await request(app).get('/api/auth/google')).status, 400)
    assert.equal((await request(app).get('/api/auth/microsoft')).status, 400)
  })
  ...
  it('OAuth callbacks redirect to /login on error', async () => {
    ...
    assert.equal((await request(app).get('/api/auth/microsoft/callback')).status, 302)
  })
})
