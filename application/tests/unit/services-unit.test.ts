...
import {
  deviceRunningConfig,
  topologyConfigBundle,
} from '../../src/services/config-export-service.js';
...
describe('config-export-service', () => {
  it('renders running-config + bundle', () => {
    const topo = buildDemoTopology();
    const cfg = deviceRunningConfig(
      topo,
      topo.nodes.find((n) => n.id === 'fw-1')!,
    );
    ...
    assert.match(
      deviceRunningConfig(
        topo,
        topo.nodes.find((n) => n.id === 'switch-1')!,
      ),
...
describe('packet-sender-service.tracePacket', () => {
  ...
  it('allowed LAN->server path', () => {
    assert.ok(
      tracePacket(topo, { srcNodeId: 'pc-1', dstNodeId: 'server-1', protocol: 'tcp', dstPort: 443 })
        .path.length >= 2,
    );
  });
  ...
  it('icmp same-switch peers', () => {
    assert.ok(
      Array.isArray(
        tracePacket(topo, { srcNodeId: 'pc-1', dstNodeId: 'pc-2', protocol: 'icmp' }).hops,
      ),
    );
  });
  it('errors on unknown nodes', () => {
    assert.equal(
      tracePacket(topo, { srcNodeId: 'pc-1', dstNodeId: 'ghost', protocol: 'tcp' }).success,
      false,
    );
  });
  ...
  it('routes between two private LANs over a two-router WAN link', () => {
    const now = Date.now();
    const ifc = (ip: string, cidr: string, name = 'eth0') => ({
      name,
      ipAddress: ip,
      subnetMask: '255.255.255.0',
      cidr,
      ...
      nodes: [
        {
          ...
          label: 'A',
          position: { x: 0, y: 0 },
          config: { interfaces: [ifc('10.1.0.10', '/24')] },
        },
        { id: 'swA', type: 'switch', label: 'swA', position: { x: 1, y: 0 }, config: {} },
        {
          ...
          config: {
            ...
            routingTable: [
              rt('10.1.0.0', '255.255.255.0', '0.0.0.0', 'connected'),
              rt('172.16.255.0', '255.255.255.252', '0.0.0.0', 'connected'),
              rt('10.2.0.0', '255.255.255.0', '172.16.255.2', 'static'),
            ],
          },
        },
        {
          ...
          config: {
            ...
            routingTable: [
              rt('10.2.0.0', '255.255.255.0', '0.0.0.0', 'connected'),
              rt('172.16.255.0', '255.255.255.252', '0.0.0.0', 'connected'),
              rt('10.1.0.0', '255.255.255.0', '172.16.255.1', 'static'),
            ],
          },
        },
        { id: 'swB', type: 'switch', label: 'swB', position: { x: 4, y: 0 }, config: {} },
        {
          ...
          label: 'B',
          position: { x: 5, y: 0 },
          config: { interfaces: [ifc('10.2.0.10', '/24')] },
        },
      ],
      edges: [
...
describe('network-service mutations', () => {
  it('covers the full node/edge surface', async () => {
    ...
    const n1 = await netdb.addNode(
      t.id,
      { type: 'pc', label: 'A', position: { x: 0, y: 0 }, config: {} },
      'unit-owner',
    );
    const n2 = await netdb.addNode(
      t.id,
      { type: 'server', label: 'B', position: { x: 1, y: 1 }, config: {} },
      'unit-owner',
    );
    assert.equal(
      (await netdb.updateNode(t.id, n1!.id, { label: 'A2' }, 'unit-owner'))?.label,
      'A2',
    );
    const e = await netdb.addEdge(
      t.id,
      { source: n1!.id, target: n2!.id, config: {} },
      'unit-owner',
    );
    assert.equal(
      (await netdb.updateEdge(t.id, e!.id, { label: 'link' }, 'unit-owner'))?.label,
      'link',
    );
    ...
    assert.equal(
      await netdb.addNode(
        'missing',
        { type: 'pc', label: 'x', position: { x: 0, y: 0 }, config: {} },
        'unit-owner',
      ),
      null,
    );
...
describe('auth-service', () => {
  it('builds OAuth urls and finds/creates users', async () => {
    ...
    const u1 = await authService.findOrCreateUser({
      provider: 'local',
      providerId: 'u@x.io',
      email: 'u@x.io',
      name: 'U',
    });
    const u2 = await authService.findOrCreateUser({
      provider: 'local',
      providerId: 'u@x.io',
      email: 'u@x.io',
      name: 'U',
  ...
  it('exchanges OAuth codes for profiles (mocked fetch)', async () => {
    ...
    globalThis.fetch = (async (url: string) => {
      ...
      return new Response(
        JSON.stringify({ sub: 'g', id: 'm', email: 'p@x.io', name: 'P', displayName: 'P' }),
        { status: 200 },
      );
    }) as typeof fetch;
    try {
