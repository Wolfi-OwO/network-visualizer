...
- [ ] **Show the routing table that produced a verdict.** The trace already knows
      ...
      _Where:_ `application/src/services/packet-sender-service.ts` (already computes
      it), `client/src/pages/network/` · _Effort:_ M
...
- [ ] **Topology templates.** Ship a few starting points — a small office, a DMZ, a
      three-tier datacenter — so a new user has something to _break_ rather than a
...
- [ ] **Search / jump-to-device.** A palette (`Ctrl+K`) that finds a device by name
      ...
      _Effort:_ S · **good first issue**

## Packet Capture

- [ ] **Export a capture as `.pcap`.** The feature is explicitly Wireshark-shaped;
      being able to _open the result in Wireshark_ is the obvious completion of that
...
- [ ] **Export/import a topology as a file.** Round-trips a network out of the app
      ...
      _Where:_ `application/src/services/config-export-service.ts` (partly exists)
      · _Effort:_ S
