# Terms of Use (AGB)

**NetViz — Network Visualizer & Simulator**  
Gültig ab: 2026-08-23

---

## 1. Akzeptanz der Bedingungen

Durch die Nutzung von NetViz akzeptieren Sie diese Allgemeinen Geschäftsbedingungen. Falls Sie nicht zustimmen, dürfen Sie die Anwendung nicht nutzen.

---

## 2. Nutzungsrechte und Einschränkungen

Sie dürfen NetViz nur für legitime, legale Zwecke nutzen.

**Verboten:**
- ❌ Netzwerk-Konfigurationen von dritten Personen ohne Erlaubnis verwenden
- ❌ Social Engineering oder Hacking-Versuche mithilfe der Anwendung
- ❌ Missbrauch für Cyberattacken oder illegale Aktivitäten
- ❌ Automatisierte Scraping oder DoS-Attacken gegen NetViz selbst
- ❌ Verbreitung von Malware oder schädlichem Code

---

## 3. User-Generated Content

Sie behalten alle Rechte an Netzwerk-Topologien, die Sie erstellen.

Der Betreiber darf Ihre Daten:
- ✅ Speichern und sichern
- ✅ Für Statistiken anonymisieren
- ❌ Nicht mit dritten teilen ohne Zustimmung
- ❌ Nicht kommerziell nutzen

---

## 4. Haftungsbeschränkung

**NetViz wird OHNE GARANTIE bereitgestellt.** Der Betreiber haftet nicht für:
- ❌ Datenverlust oder Fehler in Netzwerk-Simulationen
- ❌ Ausfallzeiten oder Unterbrechungen
- ❌ Indirekte Schäden (Gewinnverlust, etc.)
- ❌ Schäden durch externe Angriffe oder Sabotage

**Ausnahme:** Schäden durch Vorsatz oder grobe Fahrlässigkeit sind nicht ausschließbar.

---

## 5. Verfügbarkeit und Wartung

Der Service wird ohne Verfügbarkeitszusage bereitgestellt. Wartungsarbeiten können ohne vorherige Ankündigung durchgeführt werden.

---

## 6. Intellectual Property

- **NetViz Source Code:** MIT License (frei nutzbar)
- **Logos und Marken:** Alle Rechte vorbehalten
- **Ihre Topologien:** Ihr Eigentum, Sie können sie exportieren

---

## 7. Konto-Löschung

Sie können Ihr Konto jederzeit löschen. Nach 90 Tagen werden alle Daten permanent gelöscht.

Der Betreiber kann Konten sperren bei:
- Verstoß gegen diese AGB
- Illegale Aktivitäten
- Missbrauch der Plattform

---

## 8. Änderungen der Bedingungen

Diese AGB können jederzeit geändert werden. Wesentliche Änderungen werden mindestens 14 Tage im Voraus angekündigt.

---

## 9. Anwendbares Recht

Diese Bedingungen unterliegen dem **Recht der Republik Österreich**. Gerichtsstand: Österreich.

---

## 10. Kontakt

Bei Fragen zu diesen AGB:  
Email: koflerphillip@outlook.com

---

Gültig ab: 2026-08-23
